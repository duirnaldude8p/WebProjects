...
class ProfileSerializer(serializers.ModelSerializer):
	...
	def create(self, validated_data):
		...
		if section == "new profile":
			...
			prof2 = Main.objects.get(pk=1).profile_set.filter(id=12)
			for obj in prof2.iterator():
				if obj.id == 12:
					print("profiles - %s"%obj.profile_name)
			profile = Profile.objects.create(
				profile_name = validated_data['profile_name'],
				username = validated_data['username'],
				password = validated_data['password'],
				profile_pic = validated_data['profile_pic'],
				main = validated_data['main'],
				section = 'new'
              						
 			)
			return profile
		elif section == "profile name":				
			print("in update profile name")
			recieved_id = validated_data['get_id']
			for obj in profile.iterator():
				if obj.id == int(recieved_id):
					profile = Profile.objects.update(profile_name=validated_data['profile_name'])
				break
			return profile
		return profile
